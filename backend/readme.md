...
- Для **Google Sheets** (импорт через Picker): `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` (для вызова Sheets API с переданным access token), `FRONTEND_URL`. На frontend: `VITE_GOOGLE_CLIENT_ID`, `VITE_GOOGLE_API_KEY` для Google Picker и GIS (`drive.file`). В [Google Cloud Console](https://console.cloud.google.com/) включите **Google Sheets API**, **Google Drive API** и **Google Picker API**; в consent screen — scope `drive.file` (без `spreadsheets`). Импорт: пользователь выбирает таблицу в Picker, фронт передаёт краткоживущий GIS token в заголовке `x-google-picker-access-token` на `GET .../sheet-titles` и `POST .../import/google`. Опционально: миграция `drizzle/0004_google_sheets_tokens.sql` и OAuth `/auth/google/sheets` — только для legacy/экспорта; для импорта токены в БД не нужны.
