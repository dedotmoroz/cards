...
- Для **Google Sheets** (импорт/экспорт): `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI` (например `http://localhost:3000/auth/google/sheets/callback`), `FRONTEND_URL` (например `http://localhost:8888`). На frontend также нужен `VITE_GOOGLE_API_KEY` для Google Picker. **Обязательно применить миграцию** `drizzle/0004_google_sheets_tokens.sql` (таблица `google_sheets_tokens`), иначе подключение не сохранится. В [Google Cloud Console](https://console.cloud.google.com/) для проекта должны быть включены **Google Sheets API**, **Google Drive API** и **Google Picker API**. OAuth запрашивает доступ к таблицам и файлам, выбранным пользователем (`spreadsheets`, `drive.file`). После выбора файла в Google Picker фронт может передать краткоживущий GIS access token в заголовке `X-Google-Picker-Access-Token` на `GET .../sheet-titles` и `POST .../import/google`, чтобы бэкенд читал таблицу в контексте `drive.file`.
